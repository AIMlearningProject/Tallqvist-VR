        ...
        public class VoxFileImport
        {
            ...
            public static GameObject CreateGameObject(string name, VoxData data, Texture2D texture, Color32[] colors, float scale)
            {
                ...
                foreach (var entity in entities)
                {
                    ...
                    var colorsPerVertex = new Color32[allocSize * 4]; // Add colors
                    ...
                    {
                        VOXModel.CreateCubeMesh16x16(it, ref vertices, ref normals, ref uv, ref colorsPerVertex, ref triangles, ref index, scale, colors[it.material]);
                    ...
                    if (triangles.Length > 0)
                    {
                        ...
                        //meshRenderer.sharedMaterial = new Material(Shader.Find("Mobile/Diffuse")); // Original
                        ...
                        if (shader == null)
                        {
                            Debug.LogWarning("[VOX] Shader 'Mobile/Diffuse' not found. Using fallback.");
                            shader = Shader.Find("Universal Render Pipeline/Unlit");
                        }

                        if (shader == null)
                        {
                            Debug.LogError("[VOX] No valid shader found. Skipping material assignment.");
                        }
                        else
                        {
